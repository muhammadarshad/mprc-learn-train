...
def local_features(a,bit):
    """Joint two-depth 5-arm MPRC context: C + 2U + 2D + 2F + 2B."""
...
Slocal=np.zeros((len(shadow_idx),K),dtype=np.int64)
Sregional=np.zeros_like(Slocal)
Sglobal=np.zeros_like(Slocal)
...
for ci,name in enumerate(CHANNEL_NAMES):
    ...
    for bit in range(8):
        lm=local_features(atr,bit)
        tm=local_features(ate,bit)

        # LOCAL 1+8: one joint 9-bit directional state (512 possibilities).
        model_bytes+=branch_fit_score(
            lm[fit_idx],yfit,
            [(lm[val_idx],Vlocal),(lm[shadow_idx],Slocal),(tm,Tlocal)],
            512
        )
        ...
        model_bytes+=branch_fit_score(
            rg[fit_idx],yfit,
            [(rg[val_idx],Vregional),(rg[shadow_idx],Sregional),(rt,Tregional)],
            256
        )
...
wl,wr,wg=weights
shadow_scores=wl*Slocal+wr*Sregional+wg*Sglobal
shadow_accuracy=accuracy(shadow_scores,yshadow)
...
result={
    ...
    "architecture":{
        ...
        "local":{
            "geometry":"C + U1,U2 + D1,D2 + F1,F2 + B1,B2",
            "topology":"two-depth 5-arm cross; no diagonals",
            "state":"joint 9-bit directional shape",
            "states":512,
            "anchors":"7x7 stride-4 lattice with two-cell support",
    ...
    "official_test":{
        "status":"exploratory continuity benchmark; v0 already exposed this split",
    ...
    "controlled_change":"v1 changes local 1+8 geometry from an immediate square-neighbour mask to the owner's two-depth directional 5-arm cross; regional/global branches remain v0 controls.",
...
out=ROOT/"results"/"mprc_vision_cifar_v1.json"
